...
struct operation { // data structure to keep each binary operation
    ...
    string operand;
    operation(){}
    operation(const vector<string> &op): register1(op[0]), operand(op[1]), register2(op[2]) {}
};

inline bool isalnum_string(const string & s){ // checks whether a string contains only alphanumeric characters or not.
    for (auto &c:s) if (!isalnum(c)) return false;
    return true;
}

inline bool isint(const string &s){ // checks whether a string is an integer or not.
    for (auto &c:s) if (!isdigit(c)) return false;
    return true;
}

vector<string> tokenize(string s){ // tokenize a string into a vector of strings: "A add 2" --> {"A", "add", "2"}
...
int evaluate(const string & reg, const vector<operation> & data, map<string, int> & map_values, set<string> & dependent_registers){ // the main function that does the lazy evaluation recursively
    if (isint(reg)) return stoi(reg); // throws exception for out_of_range inputs!
    if (map_values.count(reg)) return map_values[reg];
    ...
    for (int i=0;i<data.size();i++){
        if (data[i].register1 != reg) continue;
        if (data[i].operand == "add") result += evaluate(data[i].register2, data, map_values, dependent_registers);
        if (data[i].operand == "subtract") result -= evaluate(data[i].register2, data, map_values, dependent_registers);
        if (data[i].operand == "multiply") result *= evaluate(data[i].register2, data, map_values, dependent_registers);        
    }

    return map_values[reg] = result;
}

bool valid_command(const vector<string> & input_vector){ // takes tokenized command and checks its validity and prints errors if needed
    if (input_vector.size() == 1){ // one argument command should only be quit
        if (input_vector[0] == "quit" || input_vector[0] == "QUIT") return true;
...
int main(int argc, char *argv[]){
    ...
    while (getline(*input_stream,input_line)){ // read input line by line (from file or stdin)
        ...
        if (!valid_command(input_vector)) continue; // check command's validity and print error messages accordingly
    ...
    if (is_input_file) input_file.close(); // closing the input file, if reading from a file.
